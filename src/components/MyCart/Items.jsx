...
const Items = ({ item }) => {
  ...
  return (
    <div className="flex justify-start 3xl:p-3 2xl:p-3 xl:p-3 lg:p-3 md:p-3 sm:p-2 xs:p-2 2xs:p-2 3xs:p-2 border-t border-slate-300 3xs:h-40">
      <span className="flex justify-center items-center mr-2">
        <input type="checkbox" checked={item.selected} onChange={() => handleSelected(item.id)} />
      </span>
      <img className="object-contain 3xs:h-16 2xs:h-20 w-40 h-40 p-2 2xs:m-auto 3xs:m-auto" src={item.image} />
      <div className="flex flex-col w-7/12 border-r border-slate-300">
        <div className="details ml-2 border-b border-slate-300">
          <Link to={`/products/${item.category}/${item.id}`} onClick={() => getSingleProduct(item.id)} className="text-lg whitespace-normal text-overflow 3xs:text-xs 2xs:text-sm">{item.title}</Link>
          <div className="priceTag flex">
            <span className="text-xs  font-semibold 3xs:text-2xs 2xs:text-2xs">$</span>
      ...
      <div className="flex-1 flex flex-col items-center ml-2 3xs:text-2xs 2xs:text-xs">
        <div className="flex pt-4 mb-3 drop-shadow-lg">
          <button
            className="bg-slate-300 w-6 3xs:w-4 rounded-l-md p-1"
            onClick={() => handleCartRemove(item)}
          >
            <BiMinus />
          </button>
            <span 
            className="w-10 3xs:w-6 text-center p-1 border-y border-slate-300"
            type="textbox"
          >
            {item.count}
          </span>
          <button
            className="bg-slate-300 w-6 3xs:w-4 rounded-r-md p-1"
            onClick={() => handleCartAdd(item)}
          >
            <BiPlus />
          </button>
        </div>
        <div className="flex-1 flex justify-center items-center 3xs:text-2xs 2xs:text-sm">
